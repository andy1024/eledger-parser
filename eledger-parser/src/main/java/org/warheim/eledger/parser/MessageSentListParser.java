...
/**
 * Parses the outgoing message list page source scraped from web
 ...
 * as they are accessible as separate web resources
  * @author andy
 */
public class MessageSentListParser implements SourcePageParser {
    ...
    public void parse(Source source, UserNotifications un) {
        ...
        for (Element msgHeader: msgHeaders) {
            ...
            String recipients = WebParserTool.noHTML(msgAttrs.get(2));
            String date = WebParserTool.noHTML(msgAttrs.get(3));
            un.putMessageSentWithoutContents(id, title, recipients, date);
