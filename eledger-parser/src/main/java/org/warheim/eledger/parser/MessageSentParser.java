...
/**
 * Parses the single outgoing message page source scraped from web
 * 
 * this page is called using ajax, so it contains just the message data, no excess html
 * @author andy
 */
public class MessageSentParser implements SourcePageParser {
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(MessageSentParser.class);
    ...
    public void parse(Source source, UserNotifications un) {
        ...
        Message msg = un.getMessageSent(source.getId());
